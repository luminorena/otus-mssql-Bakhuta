...
/*
1. —оздайте очередь дл€ формировани€ отчетов дл€ клиентов по таблице Invoices. 
ѕри вызове процедуры дл€ создани€ отчета в очередь должна отправл€тьс€ за€вка.
2. ѕри обработке очереди создавайте отчет по количеству заказов (Orders)
по клиенту за заданный период времени и складывайте готовый отчет в новую таблицу.
3. ѕроверьте, что вы корректно открываете и закрываете диалоги и у нас они не коп€тс€.
*/
...
-- процедура отправки сообщени€ в очередь - отправка заказов
...
--определ€ем сервис инициатора, таргет и контракт
