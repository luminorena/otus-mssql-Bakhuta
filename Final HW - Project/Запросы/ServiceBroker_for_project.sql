...
ALTER DATABASE VetShop SET TRUSTWORTHY ON;

ALTER AUTHORIZATION    
   ON DATABASE::VetShop TO [sa];
...
--получить сообщение от инициатора
   ...
   FROM @xml.nodes('/RequestMessage/Orders') as R(Iv);
	...
	BEGIN
		UPDATE Sales.Orders
		SET OrdersConfirmedForProcessing = GETUTCDATE(), TotalCost = Quantity*CostPerUnit
		WHERE OrderId = @OrderId;
	END;
